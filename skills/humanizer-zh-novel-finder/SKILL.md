...
```text
npx skills add klarkxy/humanizer --skill <名称> -g -y
```

4. 管理器不支持 `--skill` 时，用 `--subpath` 兜底：

```text
npx skills add klarkxy/humanizer --subpath skills/<名称> -g -y
```
...
3. 条件满足时告诉用户：
   - 先装 `humanizer-zh-novel-meta`：
     ```text
     npx skills add klarkxy/humanizer --skill humanizer-zh-novel-meta -g -y
     ```
   - 装好后说：
     ```text
